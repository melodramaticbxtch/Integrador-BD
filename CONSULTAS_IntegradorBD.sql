...
/*cliente con el monto total mas alto pero con consultas nidadas*/
SELECT 
    c.id_cliente,
    c.nombre_cliente,
    c.apellido_cliente,
    (
        SELECT SUM(cr.monto_otorgado)
        FROM Solicitud s
        JOIN Credito cr ON s.id_solicitud = cr.id_solicitud
...
WHERE (
        ...
        WHERE s2.id_cliente = c.id_cliente
            ...
            FROM (
                SELECT 
                    s3.id_cliente,
                    SUM(cr3.monto_otorgado) AS total_cliente
                FROM Solicitud s3
                JOIN Credito cr3 ON s3.id_solicitud = cr3.id_solicitud
                GROUP BY s3.id_cliente
            ) AS t
        );


/* Productos financieros con tasa superior a la media*/
SELECT 
...
/*Clientes con más solicitudes que el promedio del top 10 de clientes (*/
SELECT 
    c.id_cliente,
    c.nombre_cliente,
    COUNT(s.id_solicitud) AS solicitudes_cliente
FROM Cliente c
JOIN Solicitud s ON c.id_cliente = s.id_cliente
GROUP BY c.id_cliente
HAVING COUNT(s.id_solicitud) >
(
    SELECT AVG(top10.cant_solicitudes)
    FROM (
        SELECT s2.id_cliente, COUNT(s2.id_solicitud) AS cant_solicitudes
        FROM Solicitud s2
        GROUP BY s2.id_cliente
