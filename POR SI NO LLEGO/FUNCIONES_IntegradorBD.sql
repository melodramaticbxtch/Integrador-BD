...
CREATE FUNCTION  IF NOT EXISTS mora_total_cliente(p_id_cliente INT)
...
CREATE PROCEDURE if NOT EXISTS registrar_pago(
    IN p_id_cuota INT,
...
BEGIN
    ...
    INSERT INTO pago (id_cuota, fecha_de_pago, monto_pagado, demora, penalizacion_mora, fecha_de_alta)
    VALUES (p_id_cuota, p_fecha_pago, p_monto_pagado, v_demora, v_penalizacion, NOW());
...
CREATE PROCEDURE if NOT EXISTS aprobar_solicitud(
...
BEGIN
    ...
    INSERT INTO credito (id_producto_financiero,
                         id_solicitud,
                         monto_otorgado,
                         fecha_inicio,
                         fecha_fin,
...
CREATE PROCEDURE IF NOT EXISTS generacion_de_cuotas(
    IN p_id_credito INT,
    IN p_cuotas INT
)
BEGIN
    ...
    WHILE v_i <= p_cuotas
        DO
            INSERT INTO cuota (id_credito, numero_cuota, fecha_de_emision,
...
CREATE PROCEDURE IF NOT EXISTS reporte_cliente(
    IN p_id_cliente INT
)
BEGIN
    SELECT c.id_cliente,
...
SELECT @prod,
       ...
       @monto,
       NOW(),
       DATE_ADD(NOW(), INTERVAL 18 MONTH),
       pf.tasa_base,
       18,
       'Activo',
       NOW(),
